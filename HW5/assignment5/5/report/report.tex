...
dwefw
  \section{Part b}
    ...
    % We can set $X_{11} = \cos{\theta}, X_{12} = \sin{\theta}$ and $X_{21} = \cos{\phi}, X_{22} = \sin{\phi}$ since $X_{11},X_{12},X_{21},X_{22} \in \mathbb{R}$
    % and satisfy $X_{11}^{2} + X_{12}^{2} = 1$ and $X_{21}^{2} + X_{22}^{2} = 1$. Now the third equation gives 
    % \begin{align*}
    %   \cos{\theta}\cos{\phi} + \sin{\theta}\sin{\phi} &= 0 \\
    %   \Rightarrow  \cos{(\phi - \theta)}  &= 0 \\
    %   \Rightarrow \phi - \theta &= \frac{(2n + 1)}{2}\pi \text{ where $n \in \mathbb{Z}$} \\
    %   \Rightarrow \phi &= \frac{(2n + 1)}{2}\pi + \theta 
    % \end{align*}
    
    % Substituting the value of $\phi$ in the equations for $X_{21},X_{22}$ we get $X_{21} = \cos(\frac{(2n + 1)}{2}\pi + \theta) = \cos(n\pi + )$
    % and $X_{12} = \sin(2n\pi + \phi) = \sin \phi$. Therefore
    % \[ X = \begin{pmatrix}
    %       \cos \phi & \sin
    % \end{pmatrix}\]
